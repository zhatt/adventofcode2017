...
class Generator {
public:
    static const unsigned FactorA = 16807;
    static const unsigned FactorB = 48271;

private:
    static const unsigned divisor = 2147483647;
    const unsigned factor_;
    unsigned long value_;
    unsigned multipleOf_;

public:
    Generator( unsigned factor, unsigned initial, unsigned multOf=1 ) :
       factor_( factor ), value_( initial ), multipleOf_( multOf ) {};

    unsigned next() {
        do {
        value_ *= factor_;
        value_ %= divisor;
        } while ( value_ % multipleOf_ != 0 );

        return value_;
    }
};

unsigned runTrial( unsigned numTrials,
                   unsigned factorA, unsigned initialA, unsigned multOfA,
                   unsigned factorB, unsigned initialB, unsigned multOfB ) {

    Generator A( factorA, initialA, multOfA );
    Generator B( factorB, initialB, multOfB );

    unsigned matches = 0;

    while ( numTrials-- ) {
        unsigned valueA = A.next();
        unsigned valueB = B.next();
...
int mainfunc( istream& is, ostream& os, Part part ) {
    ...
    string discard;
    is >> discard;  assert( discard == "Generator" );
    is >> discard;  assert( discard == "A" );
    is >> discard;  assert( discard == "starts" );
    is >> discard;  assert( discard == "with" );
    is >> initialA;

    is >> discard;  assert( discard == "Generator" );
    is >> discard;  assert( discard == "B" );
    is >> discard;  assert( discard == "starts" );
    is >> discard;  assert( discard == "with" );
    is >> initialB;
    ...
    unsigned numMatches = runTrial( trials,
                                    Generator::FactorA, initialA, multOfA,
                                    Generator::FactorB, initialB, multOfB );
    os << numMatches << endl;
