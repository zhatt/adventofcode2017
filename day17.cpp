...
static int mainfunc1( istream& is, ostream& os ) {
    
    unsigned stepsPerInsert;

    is >> stepsPerInsert;
    assert( is );

    list<unsigned> buffer{ 0 };
    auto current = buffer.begin();

    for( unsigned i = 1; i <= 2017; i++ ) {
/*
        for ( auto iter = buffer.begin(); iter != buffer.end(); ++iter ) {
            if ( iter == current ) cerr << "(";
            cerr << *iter;
            if ( iter == current ) cerr << ")";
            cerr << " ";
        }
        cerr << endl;
*/
        for ( unsigned step = 0; step < stepsPerInsert; step++ ) {
            current++;
            if ( current == buffer.end() ) current = buffer.begin();
        }

        current++;
        current = buffer.insert( current, i );
...
static int mainfunc2( istream& is, ostream& os ) {
 
    unsigned stepsPerInsert;
    is >> stepsPerInsert;
    assert( is );

    unsigned calculatedCurrent = 0;
    unsigned lockListSize = 1;
    unsigned valueAfterZero = 0;

    for( unsigned i = 1; i <= 50000000; i++ ) {
        calculatedCurrent += stepsPerInsert;
        calculatedCurrent %= ( lockListSize );
...
int mainfunc( istream& is, ostream& os, Part part ) {
    ...
    if ( part == Part::PART1 ) {
        return mainfunc1( is, os );
    } else {
        return mainfunc2( is, os );
    }
}
