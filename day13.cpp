...
class Layer {
    const unsigned range_;

public:

    Layer( unsigned range ) : range_( range ) {}

    unsigned getRange() { return range_; }

    unsigned getScannerPosition( unsigned long time ) {
        unsigned pos = time  % ( ( range_ - 1 ) * 2 );
        return pos;
    }
};
...
public:
    ...
    void append( unsigned depth, unsigned range ) {
        assert( depth >= layers_.size() );

        while ( depth != layers_.size() ) {
            layers_.push_back( Layer( 0 ) );
        }

        layers_.push_back( Layer( range ) );
    }

    bool isSafe( unsigned long time, unsigned layer ) {
        bool safe = true;
        if ( layers_[layer].getRange() != 0 ) {
            unsigned position = layers_[layer].getScannerPosition( time );
            safe = position != 0;
        }
        return safe;
    }
    ...
    unsigned layerRange( unsigned depth ) {
        return layers_[depth].getRange();
    }
};
...
int mainfunc( istream& is, ostream& os, Part part ) {
    ...
    do {
        bool detected = false;
        severity = 0;
        unsigned picoSeconds = delay;

        for ( unsigned layer = 0; ! detected && layer < firewall.size(); layer++ ) {

            if ( ! firewall.isSafe( picoSeconds, layer ) ) {
                severity += layer * firewall.layerRange( layer );
                if ( part == Part::PART2 ) {
                    detected = true;
                }
            }
        ...
        if ( ! detected && part == Part::PART2 ) break;
        delay++;

    } while ( part != Part::PART1 );
