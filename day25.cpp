...
You deposit all fifty stars and reboot the printer. Suddenly, everything
seems a lot less pixelated than before.

"--raise your priority level enough to send the reboot command and... hey
look, it's printing! I'll bring it to Santa. Thanks!" She runs off.
*/
...
void loadRules( istream& is, char& beginState, unsigned& numSteps,
                Rules& rules ) {
    ...
    parser.ignore( strlen( "Begin in state " ) );
    parser >> beginState;
    assert( parser );
    }
    ...
    getline( is, line );
    istringstream parser( line );
    parser.ignore( strlen( "Perform a diagnostic checksum after " ) );
    parser >> numSteps;
    assert( parser );
    }
    ...
    while ( getline( is, line ) ) {
        ...
        // The first line read by while above is blank and gets ignored.
        ...
        for ( unsigned i = 0; i <= 1; i++ ) {
            ...
            getline( is, line );
            istringstream parser( line );
            parser.ignore( strlen( "  If the current value is " ) );
            unsigned v;
            parser >> v;
            assert( v == i );
            assert( parser );
            ...
            getline( is, line );
            istringstream parser( line );
            parser.ignore( strlen( "    - Write the value " ) );
            parser >> rule.writeValue;
            assert( parser );
            }
            ...
            getline( is, line );
            istringstream parser( line );
            parser.ignore( strlen( "    - Move one slot to the " ) );
            string tmp;
            parser >> tmp;
            if ( tmp == "left." ) {
                rule.direction = Rule::Direction::LEFT;
            } else if ( tmp == "right." ) {
                rule.direction = Rule::Direction::RIGHT;
            } else {
            ...
            getline( is, line );
            istringstream parser( line );
            parser.ignore( strlen( "    - Continue with state " ) );
            parser >> rule.nextState;
            assert( parser );
            }

            rules[ { inState, i } ] = rule;
        }
    }
}
...
int mainfunc( istream& is, ostream& os, Part part ) {
    ...
    loadRules( is, currentState, numSteps, rules );

    Tape tape;
    unsigned position = 0;

    while ( numSteps -- ) {
        unsigned currentValue = tape[ position ];
        const Rule& rule = rules[ {currentState,currentValue} ];

        tape[ position ] = rule.writeValue;
        position += static_cast<int>( rule.direction );
